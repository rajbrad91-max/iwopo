...
/**
 * 🔒 The R2 key for one File Flyer object, in the PRIVATE bucket.
 *
 * These are files a vendor and their client pass between each other, reached
 * by a share token and sometimes a password. They belong with galleries, not
 * beside the public website images.
 *
 * stored_name is generated by us, never taken from the client's filename, so
 * the only thing that could go wrong here is the vendor id — and that always
 * comes from a row already checked against the caller's token.
 */
function fileKey(vendorId, storedName) {
  return objects.keyFor(vendorId, 'files', storedName);
}
...
async function zipInto(archive, vendorId, folderId, prefix) {
  ...
  for (const it of items) {
    ...
    const o = await fileStream(it.vendor_id, it.stored_name);
...
router.post('/upload', requireAuth, upload.array('files', 30), async (req, res) => {
  ...
  try {
    ...
    for (const f of req.files) {
      ...
      /* ☁️ And to R2. Written after the disk and never fatal: the file is
         already saved locally and every reader falls back there, so an
         unreachable R2 costs a slower download rather than a lost delivery. */
...
export { storageFor, vendorDir, fileKey, fileStream };
